...
int judgeMapWeight(const Board &board,bit c){
  int res=0;
  for(int i=0;i<8;i++)
    for(int j=0;j<8;j++)
...
int judge(const Board &board,bit c){
  int res=0;
  res+=judgeMapWeight(board,c)*100;
  return res;
}
...
int main(){
  ...
  if(moves.size()){
  	...
  	vec<pair<int,int>> weightedMoves=moves.map((function<pair<int,int>(int)>)[&](int movement){
  		Board tempBoard=board;
  		tempBoard.move(col,movement);
  		return make_pair(judge(tempBoard,col),movement);
  	});
    int choosed=max_element(all(weightedMoves))->second;
